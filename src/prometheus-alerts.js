...
module.exports = {
  ...
  run: (config, cb) => {
    if (!_.has(config, 'baseUri')) { return cp(null, "Please specify Prometheus Alertmanager base URI (config.baseUri).")}

    const requestPrometheusAlertmanager = function () {
      const deferred = Q.defer();
      jsonRequest(config.baseUri + "/api/v1/alerts/groups", function (error, json) {
        if (error) {
